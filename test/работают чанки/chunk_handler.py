...
class ChunkHandler():
    ...
    def add_data(self, pcm_bytes):
        ...
        try:
            ...
            # ÐŸÐ¾ÐºÐ°Ð·Ð°Ñ‚ÑŒ Ð¿ÐµÑ€Ð²Ñ‹Ðµ Ð±Ð°Ð¹Ñ‚Ñ‹ Ð´Ð»Ñ Ð´Ð¸Ð°Ð³Ð½Ð¾ÑÑ‚Ð¸ÐºÐ¸
            self.logger.info(f"First 20 bytes: {list(pcm_bytes[:20])}")
            
            # ÐŸÐ¾Ð¿Ñ€Ð¾Ð±ÑƒÐµÐ¼ Ñ€Ð°Ð·Ð½Ñ‹Ðµ Ð¸Ð½Ñ‚ÐµÑ€Ð¿Ñ€ÐµÑ‚Ð°Ñ†Ð¸Ð¸ Ð´Ð°Ð½Ð½Ñ‹Ñ…
            try:
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ ÐºÐ°Ðº float32
                ...
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼, Ð½Ðµ Ð²ÑÐµ Ð»Ð¸ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ñ Ð½ÑƒÐ»Ð¸
                if np.all(pcm_array_f32 == 0):
                    self.logger.error("CRITICAL: All PCM values are ZERO! Audio data is silent!")
                
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ Ð´Ð¸Ð°Ð¿Ð°Ð·Ð¾Ð½ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ð¹
            ...
            try:
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ ÐºÐ°Ðº int16 (Ð²Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ñ„Ñ€Ð¾Ð½Ñ‚ Ð¾Ñ‚Ð¿Ñ€Ð°Ð²Ð»ÑÐµÑ‚ Ð² ÑÑ‚Ð¾Ð¼ Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚Ðµ)
                ...
                # Ð•ÑÐ»Ð¸ float32 Ð´Ð°ÐµÑ‚ Ð½ÑƒÐ»Ð¸, Ð½Ð¾ int16 Ð´Ð°ÐµÑ‚ Ð´Ð°Ð½Ð½Ñ‹Ðµ - Ð²Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ð¾ Ð¿Ñ€Ð¾Ð±Ð»ÐµÐ¼Ð° Ð² Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚Ðµ
    ...
    def finalize(self):
        ...
        # ÐÐ½Ð°Ð»Ð¸Ð· Ð´Ð°Ð½Ð½Ñ‹Ñ… Ð¿ÐµÑ€ÐµÐ´ ÐºÐ¾Ð½Ð²ÐµÑ€ÑÐ¸ÐµÐ¹
        chunk_stats = {
            "min": chunk_data.min(),
            "max": chunk_data.max(),
    ...
    def _pcm_to_webm(self, pcm_data, output_path):
        ...
        try:
            ...
            # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼, Ð½Ðµ Ð¿ÑƒÑÑ‚Ñ‹Ðµ Ð»Ð¸ Ð´Ð°Ð½Ð½Ñ‹Ðµ
            if len(pcm_data) == 0:
            ...
            # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ Ð°Ð¼Ð¿Ð»Ð¸Ñ‚ÑƒÐ´Ñƒ
            ...
            with tempfile.NamedTemporaryFile(suffix='.raw', delete=False) as temp_file:
                ...
                # Ð”Ð¾Ð¿Ð¾Ð»Ð½Ð¸Ñ‚ÐµÐ»ÑŒÐ½Ð°Ñ Ð¿Ñ€Ð¾Ð²ÐµÑ€ÐºÐ° Ð¿Ð¾ÑÐ»Ðµ ÐºÐ¾Ð½Ð²ÐµÑ€ÑÐ¸Ð¸
                ...
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ Ñ€Ð°Ð·Ð¼ÐµÑ€ Ð²Ñ€ÐµÐ¼ÐµÐ½Ð½Ð¾Ð³Ð¾ Ñ„Ð°Ð¹Ð»Ð°
            ...
            # Ð”ÐµÑ‚Ð°Ð»ÑŒÐ½Ð°Ñ Ð´Ð¸Ð°Ð³Ð½Ð¾ÑÑ‚Ð¸ÐºÐ° Ñ€ÐµÐ·ÑƒÐ»ÑŒÑ‚Ð°Ñ‚Ð° FFmpeg
            ...
            if result.returncode == 0:
                # ÐŸÑ€Ð¾Ð²ÐµÑ€Ð¸Ð¼ Ñ€Ð°Ð·Ð¼ÐµÑ€ Ð²Ñ‹Ñ…Ð¾Ð´Ð½Ð¾Ð³Ð¾ Ñ„Ð°Ð¹Ð»Ð°
                if os.path.exists(output_path):
                    ...
                    # ÐŸÐ¾Ð¿Ñ€Ð¾Ð±ÑƒÐµÐ¼ Ð¿Ñ€Ð¾Ð°Ð½Ð°Ð»Ð¸Ð·Ð¸Ñ€Ð¾Ð²Ð°Ñ‚ÑŒ webm Ñ„Ð°Ð¹Ð» Ñ Ð¿Ð¾Ð¼Ð¾Ñ‰ÑŒÑŽ ffprobe
                    try:
    ...
    def finalize_remaining(self):
        ...
        # ÐÐ½Ð°Ð»Ð¸Ð· Ð¾ÑÑ‚Ð°Ð²ÑˆÐ¸Ñ…ÑÑ Ð´Ð°Ð½Ð½Ñ‹Ñ…
        remaining_stats = {
